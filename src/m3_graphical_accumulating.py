...
def draw_parallel_lines(n, point, length, window):
    ...
    y = point.y
    for i in range(n):
        startpt = rg.Point(x,(y+30*i))
        endpt = rg.Point(x+length,(y+30*i))
...
def draw_lines(n, point, window):
    ...
    x = point.x
    y = point.y
    for i in range(y-100,y+101, 200//(n-1)):
        #startpt = rg.Point(x, y)
        endpt = rg.Point(x + 100, i )
        line = rg.Line(point, endpt)
        line.attach_to(window)
        print(endpt)
    window.render()
    # ------------------------------------------------------------------
